...
from internvl.model.internvl_chat import (InternVisionConfig,
                                          InternVisionModel,
                                          InternVLChatConfig,
                                          InternVLChatModelGating)
from internvl.model.internvl_chat.modeling_internvl_chat_gating import ModalityGate
...
class ModelArguments:
    ...
    enable_modality_gate: bool = field(
        default=True,
        metadata={'help': 'Enable per-image gating in InternVLChatModelGating.'},
    )
    gate_alpha: float = field(
        default=0.6,
        metadata={'help': 'Gate scaling strength. Range is roughly [1-0.5*alpha, 1+0.5*alpha].'},
    )
    gate_center: float = field(
        default=0.8,
        metadata={'help': 'Gate center for scaling, e.g., scale = 1 + alpha * (g - gate_center).'},
    )
    gate_hidden_ratio: float = field(
        default=0.5,
        metadata={'help': 'Hidden ratio for the gating MLP.'},
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        print(f'[Dataset] tokenizer.model_max_length: {self.max_tokens}')
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        logger.info('Loading InternVLChatModelGating...')
        ...
        config.enable_modality_gate = model_args.enable_modality_gate
        config.gate_alpha = model_args.gate_alpha
        config.gate_hidden_ratio = model_args.gate_hidden_ratio
        config.gate_center = model_args.gate_center
        ...
        internvl_chat_config.enable_modality_gate = model_args.enable_modality_gate
        internvl_chat_config.gate_alpha = model_args.gate_alpha
        internvl_chat_config.gate_hidden_ratio = model_args.gate_hidden_ratio
        internvl_chat_config.gate_center = model_args.gate_center
        logger.info('Building InternVLChatModelGating...')
        model = InternVLChatModelGating(internvl_chat_config, vision_model, llm)

    gate_module = ModalityGate(
        hidden_size=model.config.llm_config.hidden_size,
        gate_hidden_ratio=model_args.gate_hidden_ratio,
    )
    model.load_gating_module(gate_module)
