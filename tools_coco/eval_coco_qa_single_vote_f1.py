"""
Evaluation script for single-image CT medical VLM.
...
Data format:
- question_id: "{patient_id}/{modality}/images/{slice_num}" or similar
- Each patient has multiple slices, each slice has multiple question types (QA1-QA4)
"""
...
def extract_patient_id(question_id: str, gt_entry: Dict) -> str:
    ...
    Format examples:
    - "589023/AP/images/24" -> patient_id = "589023"
    - With dataset prefix in image: "liver_MRI_EXTERNAL1_HCC/589023/AP/images/24.png"
    """
    # If image field has dataset prefix, extract patient from there
    if 'image' in gt_entry:
        image_path = gt_entry['image']
        # Format: "dataset/patient_id/modality/images/slice.png"
        parts = image_path.split('/')
        if len(parts) >= 2:
            # parts[0] is dataset, parts[1] is patient_id
            return parts[1]
...
def evaluate_single_qa(pred_file: str, gt_file: str):
    """Main evaluation function for single-modal QA with patient-level voting"""

    print("=" * 70)
    print("Single-Image Medical VLM Evaluation")
    print("=" * 70)
    ...
    # 2. By patient + question_type (for patient-level voting)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate single-modal medical QA with patient-level voting")
    ...
    evaluate_single_qa(args.pred, args.gt)
