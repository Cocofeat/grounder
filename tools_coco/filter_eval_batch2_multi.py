#!/usr/bin/env python3
"""
Evaluate batch2 multimodal QA with two-level filtering:
...
def eval_dataset(pred_path, gt_path, min_per_option, no_patient_skip):
    ...
    # Stage 1: slice-level filtering, then vote
    voted = []
    incorrect_pool = []
    ...
    # Stage 2: patient-level filtering (optional)
    if no_patient_skip:
...
def collect_datasets():
    return sorted([p for p in DATA_BASE.glob("liver_MRI_EXTERNAL*_ready_multimodal") if p.is_dir()])


def main():
    parser = argparse.ArgumentParser(description="Filter-eval batch2 multimodal QA.")
    ...
    for ds in collect_datasets():
        ds_name = ds.name
        pred = BASE_DIR / "playground/coco" / f"external_batch2_multi_{ds_name}_qa.json"
        pred_bbox = BASE_DIR / "playground/coco" / f"external_batch2_multi_{ds_name}_qa_with_bbox.json"
        gt = ds / "test_QA_EN_multimodal_4mod.json"
        gt_bbox = ds / "test_QA_EN_multimodal_4mod_with_bbox.json"
    ...
    print("Batch2 Multi QA (filtered, patient-level)")
    ...
    print("\nBatch2 Multi QA_with_bbox (filtered, patient-level)")
    print("| dataset | question | acc | f1 | n |")
    print("|---|---|---|---|---|")
