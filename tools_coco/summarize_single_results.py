#!/usr/bin/env python3
"""
Summarize single-model results for QA, report, and grounding.

Notes:
- QA metrics are patient-level (majority vote across slices), with per-QA and overall.
- "external" + "external_GXMU_bn" are merged into a single dataset "external_merged".
- Outputs CSVs for easy reuse.
"""
...
def eval_qa_single(pred_path, gt_path, dataset_prefix=None):
...
def eval_qa_single_multi(datasets):
    data_by_patient_qtype = defaultdict(lambda: {"pred": [], "gt": []})
...
def eval_report_single(pred_path, gt_path, compute_metrics, dataset_prefix=None):
...
def eval_report_single_multi(datasets, compute_metrics):
    if compute_metrics is None:
        return None
    data_by_patient = defaultdict(lambda: {"preds": [], "gts": []})
    for ds_name, (pred_path, gt_path) in datasets:
...
def eval_grounding_single_multi(datasets):
    from tools_coco.eval_grounding import (
        parse_bounding_box_single,
        calculate_iou,
        load_ground_truth as load_ground_truth_grounding,
    ...
    for ds_name, (pred_path, gt_path) in datasets:
        ...
        gt_data, _, _ = load_ground_truth_grounding(gt_path)
        for qid, label in gt_data.items():
            gts[f"{ds_name}:{qid}"] = label
    ...
    for qid in common_ids:
        pred_box = parse_bounding_box_single(preds[qid])
        gt_box = parse_bounding_box_single(gts[qid])
        if pred_box is None or gt_box is None:
            invalid_count += 1
            continue
        iou = calculate_iou(pred_box, gt_box)
...
def main():
    parser = argparse.ArgumentParser(description="Summarize single model results.")
    parser.add_argument(
        "--output-dir",
        default="evaluation_results/single_summary",
    ...
    qa_jobs = {
        "external": ("playground/coco/external1_single_qa.json",
                     "/mnt/data/by/data/coco_new/labels_2025_11_29/external/test_QA_EN_external.json"),
        "external_GXMU_bn": ("playground/coco/external_GXMU_bn_single_qa.json",
                             "/mnt/data/by/data/coco_new/labels_2025_11_29/external_GXMU_benign_normal/test_QA_EN.json"),
        "external_ENSHI": ("playground/coco/external_ENSHI_single_qa.json",
                           "/mnt/data/by/data/coco_new/labels_2025_11_29/external_ENSHI/test_QA_EN.json"),
        "external_SanYa": ("playground/coco/external_SanYa_single_qa.json",
                           "/mnt/data/by/data/coco_new/labels_2025_11_29/external_SanYa/test_QA_EN.json"),
    }

    qa_bbox_jobs = {
        "external": ("playground/coco/external1_single_qa_with_bbox.json",
                     "/mnt/data/by/data/coco_new/labels_2025_11_29/external/test_QA_EN_external_with_bbox.json"),
        "external_GXMU_bn": ("playground/coco/external_GXMU_bn_single_qa_with_bbox.json",
                             "/mnt/data/by/data/coco_new/labels_2025_11_29/external_GXMU_benign_normal/test_QA_EN_with_bbox.json"),
        "external_ENSHI": ("playground/coco/external_ENSHI_single_qa_with_bbox.json",
                           "/mnt/data/by/data/coco_new/labels_2025_11_29/external_ENSHI/test_QA_EN_with_bbox.json"),
        "external_SanYa": ("playground/coco/external_SanYa_single_qa_with_bbox.json",
                           "/mnt/data/by/data/coco_new/labels_2025_11_29/external_SanYa/test_QA_EN_with_bbox.json"),
    }

    report_jobs = {
        "external": ("playground/coco/external1_single_report.json",
                     "/mnt/data/by/data/coco_new/labels_2025_11_29/external/test_report_generation_EN.json"),
        "external_GXMU_bn": ("playground/coco/external_GXMU_bn_single_report.json",
                             "/mnt/data/by/data/coco_new/labels_2025_11_29/external_GXMU_benign_normal/test_report_generation_EN.json"),
        "external_ENSHI": ("playground/coco/external_ENSHI_single_report.json",
                           "/mnt/data/by/data/coco_new/labels_2025_11_29/external_ENSHI/test_report_generation_EN.json"),
        "external_SanYa": ("playground/coco/external_SanYa_single_report.json",
                           "/mnt/data/by/data/coco_new/labels_2025_11_29/external_SanYa/test_report_generation_EN.json"),
    }

    report_bbox_jobs = {
        "external": ("playground/coco/external1_single_report_with_bbox.json",
                     "/mnt/data/by/data/coco_new/labels_2025_11_29/external/test_report_generation_EN_with_bbox.json"),
        "external_GXMU_bn": ("playground/coco/external_GXMU_bn_single_report_with_bbox.json",
                             "/mnt/data/by/data/coco_new/labels_2025_11_29/external_GXMU_benign_normal/test_report_generation_EN_with_bbox.json"),
        "external_ENSHI": ("playground/coco/external_ENSHI_single_report_with_bbox.json",
                           "/mnt/data/by/data/coco_new/labels_2025_11_29/external_ENSHI/test_report_generation_EN_with_bbox.json"),
        "external_SanYa": ("playground/coco/external_SanYa_single_report_with_bbox.json",
                           "/mnt/data/by/data/coco_new/labels_2025_11_29/external_SanYa/test_report_generation_EN_with_bbox.json"),
    }

    grounding_jobs = {
        "external": ("playground/coco/external1_single_grounding.json",
                     "/mnt/data/by/data/coco_new/labels_2025_11_29/external/test_bbox.json"),
        "external_GXMU_bn": ("playground/coco/external_GXMU_bn_single_grounding.json",
                             "/mnt/data/by/data/coco_new/labels_2025_11_29/external_GXMU_benign_normal/test_bbox.json"),
        "external_ENSHI": ("playground/coco/external_ENSHI_single_grounding.json",
                           "/mnt/data/by/data/coco_new/labels_2025_11_29/external_ENSHI/test_bbox.json"),
        "external_SanYa": ("playground/coco/external_SanYa_single_grounding.json",
                           "/mnt/data/by/data/coco_new/labels_2025_11_29/external_SanYa/test_bbox.json"),
    }
    ...
    for ds, (pred_path, gt_path) in qa_jobs.items():
        ...
        if pred_full.exists() and Path(gt_path).exists():
            qa_results[ds] = eval_qa_single(str(pred_full), gt_path, dataset_prefix=ds)

    for ds, (pred_path, gt_path) in qa_bbox_jobs.items():
        pred_full = BASE_DIR / pred_path
        if pred_full.exists() and Path(gt_path).exists():
            qa_bbox_results[ds] = eval_qa_single(str(pred_full), gt_path, dataset_prefix=ds)

    merge_key = "external_merged"
    if "external" in qa_jobs and "external_GXMU_bn" in qa_jobs:
        qa_results[merge_key] = eval_qa_single_multi([
        ...
        qa_bbox_results[merge_key] = eval_qa_single_multi([
    ...
    if compute_metrics is not None:
        ...
        if "external" in report_jobs and "external_GXMU_bn" in report_jobs:
            ...
            report_bbox_results[merge_key] = eval_report_single_multi([
    ...
    for ds, (pred_path, gt_path) in grounding_jobs.items():
        pred_full = BASE_DIR / pred_path
        if pred_full.exists() and Path(gt_path).exists():
            grounding_results[ds] = eval_grounding_single(str(pred_full), gt_path)

    if "external" in grounding_jobs and "external_GXMU_bn" in grounding_jobs:
        grounding_results[merge_key] = eval_grounding_single_multi([
    ...
    print_qa_table("QA (single, patient-level)", qa_results)
    print_qa_table("QA_with_bbox (single, patient-level)", qa_bbox_results)
    if compute_metrics is None:
        print("Report metrics skipped (compute_metrics unavailable).")
    else:
        print_report_table("Report (single, patient-level)", report_results)
        print_report_table("Report_with_bbox (single, patient-level)", report_bbox_results)
    print_grounding_table("Grounding (single, overall)", grounding_results)
